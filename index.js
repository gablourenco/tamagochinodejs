...
function getHungry() {
    ...
    if(state.hungry < 100 && state.hungry > 50) {
        return   '(￣﹃￣) j\'ai faim, appuie sur "f" pour me donner a manger 🍗 '  + state.hungry  
    }
    if(state.hungry < 50 && state.hungry > 0) {
        return   ' ԅ(¯﹃ ¯ԅ) a l\'aide, je suis en train de crever de faim, appuis sur "f" 🍗 ' + state.hungry 
    }
    if(state.hungry === 0) {
        return '(x﹃x)'
    }
...
function getClean() {
    ...
    if(state.clean < 170) {
        return '(◕ﻌ◕*) je ne suis pas sale mais tu peux me laver avec la touche "c"  🚿 ' + state.clean
    }
    if(state.clean < 100) {
        return ' (ತ◞౪◟ತ‵) je suis sale , appuie sur "c" pour me laver' + state.clean
    }
}
function setClean() {
    if(state.clean < 190){
